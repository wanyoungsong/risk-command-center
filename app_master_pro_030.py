...
def stream_ai_briefing(total_pnl, els_pnl, bond_pnl, top_driver, var_pct, kg_context):
    ...
    gemini-3.1-flash-lite-preview 모델에게 경영진 브리핑 스트리밍 출력을 요청하는 제너레이터 함수
    """
...
def stream_ai_prescription(dept_name, usage_pct, metric_name, exposure_amt, limit_amt, kg_context):
    ...
    gemini-3.1-flash-lite-preview 모델에게 AI 처방전 스트리밍 출력을 요청하는 제너레이터 함수
    """
    model = genai.GenerativeModel('gemini-3.1-flash-lite-preview')
...
if main_menu == "1. 전사 리스크 대시보드" and sub_menu == "1-1. 전사 마켓 리스크 브리핑":
    ...
    st.caption("수학 엔진의 정량적 연산 결과(Fact)와 사내 지식 그래프(Neo4j Aura)의 온톨로지 지식(사내 규정 및 인과관계)을 결합하여 gemini-3.1-flash-lite-preview 모델이 실시간으로 경영진 보고서를 작성합니다.")
    ...
    with col_b1:
        # AI 브리핑 생성 버튼
        if st.button("✨ 경영진 보고용 실시간 AI 브리핑 생성 (gemini-3.1-flash-lite-preview)", type="primary", use_container_width=True):
