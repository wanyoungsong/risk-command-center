...
if main_menu == "1. 전사 리스크 대시보드" and sub_menu == "1-1. 전사 마켓 리스크 브리핑":
    ...
    with col_b1:
        ...
        st.warning(f'''
        **[Warning] 전사 리스크 한도 소진율 {var_usage_pct:.1f}% 도달**

        금일 {top_risk_driver} 팩터의 변동성 급등 및 KOSPI200 체결강도 하락에 따라, ELS 포트폴리오에서 **비선형 리스크(Vega) 및 유동성 패널티가 크게 확대**되었습니다.

        해당 포트폴리오의 평가손실({daily_els_pnl / 100000000:,.1f}억)이 전사 일간 P&L 하락의 핵심을 견인하고 있습니다. 동시에 국채 금리 변동성 확대로 인해 채권 포트폴리오의 금리 민감도(Rho 노출도)가 증가하여 전사적 손실 폭이 확대되었습니다.

        **[AI 권고 조치]** 사내 리스크 관리 규정에 의거하여, ELS 포트폴리오의 Vega 중립(Neutral)을 위한 옵션 헤지 비중 즉각 확대 및 채권 듀레이션 갭 축소 검토 요망.
        ''')
    ...
    with col_b2:
        ...
        net_rca.add_node("Macro_Rate", label=f"{top_risk_driver}\n변동성 확대", color="#ffb3b3", size=20)
        net_rca.add_node("Macro_Vol", label="시장 내재변동성(IV) 폭등", color="#ffb3b3", size=20)

        net_rca.add_node("Port_Bond", label=f"Bond_Portfolio\n({daily_bond_pnl / 100000000:,.0f}억)", color="#cce5ff", size=20)
        net_rca.add_node("Port_ELS", label=f"ELS_Portfolio\n({daily_els_pnl / 100000000:,.0f}억)", color="#cce5ff", size=25)

        net_rca.add_node("PnL", label=f"전사 일간 P&L\n({daily_total_pnl / 100000000:,.0f}억)", color="#ff9999", size=35)
        ...
        net_rca.add_edge("Port_Bond", "PnL", arrows="to", color="#ff9999")
        net_rca.add_edge("Port_ELS", "PnL", label="주요 원인", arrows="to", color="#ff9999", value=2)
...
# ==========================================
    ...
    with col_t1:
        ...
        with st.container(border=True):
            ...
            # 한도 소진율에 따른 동적 프로그레스 바 색상 및 경고 문구
    ...
    with col_t2:
        ...
        with st.container(border=True):
            # 채권 데스크의 핵심은 금리 민감도(Rho) 한도
            rho_exposure = var_sens.filter(like='KTB').sum() / 100000000 # 억 원
            rho_limit = 40.0 # 가상의 한도 (40억)
            rho_usage = abs(rho_exposure / rho_limit) * 100
            
            st.markdown("**Rho (금리 민감도) 한도**: 40억 / 1bp")
            if rho_usage >= 100:
                st.progress(1.0, text=f"🚨 {rho_usage:.1f}% (한도 초과)")
            else:
                st.progress(min(rho_usage / 100.0, 1.0), text=f"⚠️ {rho_usage:.1f}% (주의)")
    ...
    with col_t3:
        ...
        with st.container(border=True):
            # ELS 데스크의 핵심은 변동성 민감도(Vega) 한도
            vega_exposure = var_sens.filter(like='Vol').sum() / 100000000 # 억 원
            vega_limit = 30.0 # 가상의 한도 (30억)
            vega_usage = abs(vega_exposure / vega_limit) * 100
            
            st.markdown("**Vega (변동성 민감도) 한도**: 30억 / 1%p")
            if vega_usage >= 100:
                st.progress(1.0, text=f"🚨 {vega_usage:.1f}% (한도 초과)")
            else:
                st.progress(min(vega_usage / 100.0, 1.0), text=f"❌ {vega_usage:.1f}% (초과 임박)")
                
            st.markdown(f"**현재 Net P&L (헤지비용 포함)**: {daily_els_pnl / 100000000:,.1f}억 원")
            
    st.markdown("---")
    st.info("💡 **시스템 알림**: 한도 모니터링은 장중 실시간 매매 데이터 및 기초자산 가격을 반영하여 1시간 주기로 갱신됩니다. 현재 ELS 데스크의 Vega 한도와 전사 VaR 한도가 사내 리스크 관리 규정(제14조)의 임계치를 상회하고 있습니다.")
    ...
    with col_left:
        ...
        if st.session_state.scenario_step >= 1:
            ...
            with st.container(border=True):
                ...
                st.caption("- 참조 출처: 블룸버그 매크로 리포트 (최근 7일), IB 원자재 전망 보고서")
                st.write("유가(WTI)가 배럴당 $90를 돌파함에 따라 글로벌 인플레이션 반등 우려가 심화되고 있습니다. 이에 따라 미 연준(Fed)의 금리 인하 지연 전망이 지배적이며, 신흥국(EM) 및 반도체 섹터 자본 유출 리스크가 부각되고 있습니다.")

                st.markdown("**🕸️ 지식 그래프 인과관계 변환**")
                st.markdown('''
                1. **유가 급등** ➡ 글로벌 물가 상승 압력 ➡ **전 구간 금리 동반 상승 (Target: +100bp)**
                2. **외국인 자금 이탈** ➡ 투심 악화 및 유동성 경색 ➡ **KOSPI 200 하락 (Target: 75% 수준)**
                3. **반도체 섹터 침체** ➡ 대형주 동반 하락 ➡ **삼성전자 하락 (Target: 55% 수준)**
                ''')
            ...
            # 표 형태의 파라미터 에디터 (새로운 포트폴리오 팩터에 맞게 수정)
            scenario_df = pd.DataFrame({
                "리스크 팩터": ["국채/회사채 금리", "KOSPI 200 지수", "삼성전자 주가"],
                "현재 수준": ["Base Rate", "100%", "100%"],
                "최대 충격 (Target)": ["+100 bp", "25% 하락 (75%)", "45% 하락 (55%)"],
